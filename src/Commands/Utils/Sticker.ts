...
@Command('sticker', {
    description: 'Converts video/image/gif to sticker',
    category: 'utils',
    usage: 'sticker [quote/caption image/video/gif] | [pack_name] | [author]',
    exp: 10,
    cooldown: 15,
    aliases: ['s']
})
export default class command extends BaseCommand {
    override execute = async (M: Message, { flags, context }: IArgs): Promise<void> => {
        ...
        const sticker = new Sticker(buffer, {
            categories,
            pack: pack[1] ? pack[1].trim() : '🌠 Shooting',
            author: pack[2] ? pack[2].trim() : 'Star 🌠',
            quality,
            type:
